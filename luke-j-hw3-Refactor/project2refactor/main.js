...
{
    document.querySelector("#searchbox").classList.add("is-loading");
    console.log(document.querySelector("#searchbox").classList)
...
async function recsLoaded(e)
{
    ...
    recommendations.forEach(sR => {
        bigString = `<div class="hero is-large is-info p-2 has-background-link mb-6"> <a href=${sR.url}>
        <img src="${sR.images.jpg.image_url}"></a>
        <a href=${sR.url}>
        <p class="title is-size-3 mb-1">${sR.title}</p></a><p>Airing:&nbsp; ${sR.status}</p><p>Score:&nbsp;${sR.score}</p><p>Episodes:&nbsp; ${sR.episodes}</p><br></div>`;
         document.querySelector("#results").innerHTML += bigString; 
    })
}
